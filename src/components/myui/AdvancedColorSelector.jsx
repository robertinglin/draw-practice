...
const onMove = (x, y, w, h) => {
  ...
  const drawWheel = useCallback(() => {
    ...
    ctx.beginPath();
    ctx.arc(
      center + indicatorRadius * Math.cos(hueAngle),
      center + indicatorRadius * Math.sin(hueAngle),
      5, 0, 2 * Math.PI
    );
    ctx.fillStyle = 'white';
    ctx.fill();
  ...
  return (
    <DraggableContainer initialX={colorPosition.x} initialY={colorPosition.y}
      minWidth={260} minHeight={300}
      initialHeight={colorPosition.h} initialWidth={colorPosition.w}
    onMove={onMove}>
      <div>Advanced Color Selector</div>
      <div className="flex">
        <div>
        ...
        <div className="ml-4 flex flex-col space-y-1">
          {quickSwatches.map((color, index) => (
            <div
              key={index}
              className="w-5 h-5 rounded-sm cursor-pointer border border-gray-600"
